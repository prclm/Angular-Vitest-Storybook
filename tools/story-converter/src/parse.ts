/**
 * Story Parser
 * Parses TypeScript story files to extract story definitions
 */
...
export function parseStoryFile(filePath: string): StoryMetadata {
  ...
  ts.forEachChild(sourceFile, (node) => {
    // Look for "const meta: Meta<Button> = {...}"
    if (
    ...
    node.properties.forEach(prop => {
      if (
        ...
        if (propName === 'title' && ts.isStringLiteral(prop.initializer)) {
          ...
          // Handle both Identifier (Button) and other expression types
          if (ts.isIdentifier(prop.initializer)) {
            metadata.component = prop.initializer.text;
          } else {
            // Extract text from source code as fallback
